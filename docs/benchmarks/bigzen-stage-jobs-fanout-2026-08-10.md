...
The current five-Job pipeline and the detection fan-out/fan-in path passed on
BIGZEN at commit `74b6d7ad3aa5471f9a97a67a5d8fdb999abe4aa7`. The run observed the
derived 8, 12 and 24 GB GPU selectors on real K3s Jobs, published strict
Artifact Manifest v2 workspaces, processed a raster plan above the former
4,096-tile ceiling, persisted one receipt per shard and completed through a
separate finalizer Job.
...
| Application commit | `74b6d7ad3aa5471f9a97a67a5d8fdb999abe4aa7` |
...
- Treat the ten SAM3 features as pipeline evidence only; this run did not
  provide labelled precision/recall ground truth.
