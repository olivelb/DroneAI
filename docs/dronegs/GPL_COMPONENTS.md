...
| DroneGS MRNF growth, selection, edge, optimizer, progressive-SH, and topology-lifecycle adaptation | LichtFeld `src/training/strategies/mrnf.cpp`, `src/training/strategies/strategy_utils.cpp`, `src/training/optimizer/adam_optimizer.cpp`, `src/training/rasterization/fastgs/optimizer/src/adam_api.cu`, `src/training/kernels/mrnf_kernels.cu`, `src/training/kernels/densification_kernels.cu`, `src/training/rasterization/edge_rasterizer.cpp`, `src/training/rasterization/edge_compute/rasterization/include/kernels_forward.cuh`, `src/training/rasterization/fastgs/rasterization/include/kernels_backward.cuh`, and `src/training/trainer.cpp` at `1004c0841a3776e3f67866ff34101fbc9677397f`; local `app1-colmap/dronegs/cuda/rasterization.cu` and `app1-colmap/dronegs/cuda/trainer.cu`, modified 2026-07-25 through 2026-07-26 | GPL-3.0-or-later | Dev.15 error-map weighting, cadence, capacity growth, and split; dev.16 weighted-Gumbel and edge guidance; dev.17 optimizer rates, epsilon, bounds normalization, and schedules; dev.18 dual profiles and effective-update telemetry; dev.19 per-family epsilon and exact one-family ablations; dev.20 strict DC-plus-opacity combination; dev.21 intermediate-DC-plus-opacity calibration profiles; dev.22 two-scene validation; dev.24 progressive SH cadence and optimizer integration; dev.25 prune/reuse, deterministic means noise, opacity/scale decay, and hard compaction preserving optimizer state; dev.27 native-sm_89 compact projected-record sorting, dev.28 Ada radix/occupancy tuning, and dev.29 shared-memory backward batching inside the same GPL translation unit | Both combined CUDA translation units carry GPL SPDX/copyright headers; distribute the resulting native binary as GPL-covered, retain this modification record, and make corresponding source available |
