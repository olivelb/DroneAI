...
| gsplat AbsGrad reference | Official `nerfstudio-project/gsplat` at `2b902ff1891fc7f73f0f9b8c8bfc932cef2b198c`; `gsplat/cuda/csrc/RasterizeToPixels3DGSDevice.cuh` and `gsplat/strategy/default.py` inspected 2026-07-26 | Apache-2.0 | Dev.36 algorithmic validation of per-pixel absolute projected-mean gradients and per-view accumulation | No gsplat source is linked or vendored. DroneGS independently adapts the published AbsGS mechanism to its existing GPL rasterizer, retains this exact reference revision, and must retain the Apache notice if source is copied later |
