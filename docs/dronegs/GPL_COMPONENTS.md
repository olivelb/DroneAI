...
| DroneGS native trainer | `app1-colmap/dronegs/` | MIT; original DroneAI code | Fixed-topology anisotropic ordered-alpha CUDA training prototype | No LichtFeld source copied; Phase 4 JPEG/scaled-IDCT orchestration, bounded decode queue/cache, anisotropic covariance projection/conics, analytical position/log-scale/quaternion backward, rasterizer, persistent context, and Adam files are original; record any future adapted file separately before import |
