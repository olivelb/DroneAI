...
| DroneGS native trainer | `app1-colmap/dronegs/` | MIT; original DroneAI code | Fixed-topology anisotropic geometry-optimized ordered-alpha CUDA prototype | No LichtFeld source copied; Phase 4 JPEG/scaled-IDCT orchestration, bounded decode queue/cache, anisotropic covariance projection/conics, analytical geometry and DSSIM backward, persistent Adam/schedules, held-out split, CUDA PSNR/SSIM, rasterizer, and training context files are original; dev.13/dev.14 match documented split, metric, and L1+DSSIM behavior through independent implementations and record the GPL control separately; record any future adapted file before import |
