...
| LichtFeld minimal and deterministic-replay patches | `app1-colmap/patches/lichtfeld-*.patch` | Modifications to GPL work | Headless/minimal build changes and opt-in `LFS_SAMPLER_SEED` sampling, asynchronous-result ordering, and MRNF random seeds | Treat patches and resulting binary as GPL-covered modifications |
| LichtFeld runtime image | `app1-colmap/Dockerfile.lichtfeld` and optional `--with-lichtfeld` local image | Contains GPL binary | Explicit rollback subprocess runtime | Bundle notices and ensure corresponding source is obtainable; it is not part of the default DroneGS image |
