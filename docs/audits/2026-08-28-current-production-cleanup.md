...
| Deployment topology | Standalone map analysis creation/retry still emits orthomosaic Kafka events consumed only by the fused processing/IA workers. Production overlays disable those workers, and no corresponding Stage Job consumer was found. Do not delete the only analysis execution path without deciding whether to migrate or retire this feature. |
