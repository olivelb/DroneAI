...
| Strict typing for `gaussian_ortho` | Confirmed absent from the current mypy ratchet. | Seventeen modules now run under strict mypy, including explicit COLMAP binary, EXIF/GPS, Gaussian-model, orthophoto-renderer and end-to-end orchestration boundaries. |
