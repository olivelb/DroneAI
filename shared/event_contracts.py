    ...
    *,
    ...
    if expected_type and event_type != expected_type:
        raise EventValidationError(
            f"expected event_type={expected_type}, got {event_type}"
        )
    ...
    if event_type == "orthomosaic" and not (
        event.get("ortho_s3_key") or event.get("ortho_path")
    ):
        missing.append("ortho_s3_key|ortho_path")
    if event_type == "image_tile" and not (
        event.get("tile_s3_key") or event.get("tile_path")
    ):
    ...
    normalized.setdefault("emitted_at", datetime.now(timezone.utc).isoformat())
