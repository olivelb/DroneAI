...
usage() {
  printf 'Usage: %s [--check] <zone> <IPv4> <subdomain>...\n' "${0##*/}" >&2
}
...
for subdomain in "${subdomains[@]}"; do
  ...
  if [[ ${record_count} -gt 1 ]]; then
    printf 'Refusing to modify %s.%s: %s A records already exist.\n' \
  ...
  if [[ ${record_count} -eq 0 ]]; then
    printf '%s %s.%s -> %s\n' \
      "$([[ ${check_only} == true ]] && printf WOULD-CREATE || printf CREATE)" \
      "${subdomain}" "${zone}" "${target}"
    if [[ ${check_only} == false ]]; then
      body=$(jq -cn \
        --arg subdomain "${subdomain}" --arg target "${target}" \
        '{fieldType:"A",subDomain:$subdomain,target:$target,ttl:300}')
      api_call POST "/domain/zone/${zone}/record" "${body}" >/dev/null
      changed=true
    fi
    continue
  fi
  ...
  current_ttl=$(jq -r '.ttl' <<<"${record}")

  if [[ ${current_target} == "${target}" && ${current_ttl} == 300 ]]; then
    printf 'UNCHANGED %s.%s -> %s (TTL 300)\n' "${subdomain}" "${zone}" "${target}"
    continue
  fi

  printf '%s %s.%s: %s/TTL%s -> %s/TTL300\n' \
    "$([[ ${check_only} == true ]] && printf WOULD-UPDATE || printf UPDATE)" \
    "${subdomain}" "${zone}" "${current_target}" "${current_ttl}" "${target}"
  if [[ ${check_only} == false ]]; then
    body=$(jq -cn --arg target "${target}" '{target:$target,ttl:300}')
    api_call PUT "/domain/zone/${zone}/record/${record_id}" "${body}" >/dev/null
    changed=true
  fi
done
