...
export const decodeGsTileManifest = (value: unknown): GsTileManifest => {
  ...
  if (manifest.nodes.length === 0 || manifest.packs.length === 0) {
    throw new ResponseContractError(
      "GSTile manifest",
      "$",
  ...
  manifest.packs.forEach((pack) => {
    ...
    if (packIds.has(pack.id) || pack.byteLength < 32 || pack.recordCount < 1) {
      throw new ResponseContractError(
        "GSTile manifest",
        "$.packs",
  ...
  manifest.nodes.forEach((node) => {
    ...
    if (node.tile && !packIds.has(node.tile.pack)) {
      throw new ResponseContractError(
        "GSTile manifest",
        `$.nodes.${node.id}.tile.pack`,
        "known pack id",
      );
    }
  });
