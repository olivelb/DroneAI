...
async function run(){
  ...
  try{
    ...
    report.edgeOpacityCases = [];
    // Change only the production setter: no pose, LOD or work-buffer update.
    // Let the on-demand renderer settle before testing stationary redraw.
    for (let i = 0; i < 12; i++) { backend.render(performance.now()); await backend.benchmarkQueueDone(); }
    for (const value of [0.25, 1, 2, 1]) {
      backend.setEdgeOpacity(value);
      backend.render(performance.now()); await backend.benchmarkQueueDone();
      report.edgeOpacityCases.push({value, capture:$('viewer').toDataURL('image/png'),
        snapshot:backend.benchmarkSnapshot()});
      for (let i = 0; i < 12; i++) { backend.render(performance.now()); await backend.benchmarkQueueDone(); }
    }
    report.invalidEdgeOpacityRejected = [];
    for (const value of [0, 2.01, NaN, Infinity]) {
      try { backend.setEdgeOpacity(value); throw Error('Invalid edge opacity accepted'); }
      catch(error) { if (!(error instanceof RangeError)) throw error; report.invalidEdgeOpacityRejected.push(String(value)); }
    }
