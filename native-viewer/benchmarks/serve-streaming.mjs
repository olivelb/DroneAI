...
for(const p of manifest.packs) {
  for(const kind of ['base','sh']) if(p.streams) packs.set('/bundle/'+p.streams[kind].path,resolve(bundle,p.streams[kind].path));
  delete p.encodings;
} // same canonical Q96 bytes, no transport comparison
const server=http.createServer(async(req,res)=>{
  try{
    const pathname=new URL(req.url,'http://localhost').pathname;
    if(pathname==='/results'&&req.method==='POST'){
      let text='';for await(const chunk of req){text+=chunk;if(text.length>64*1024**2)throw Error('Report too large');}
...
server.listen(8770,'127.0.0.1',()=>console.log('GSTile comparison: http://127.0.0.1:8770/?dev=1'));
