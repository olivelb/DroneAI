...
def _read_colmap_dense_array(path: str) -> np.ndarray:
    ...
    data = data[:expected].reshape((height, width, channels))
...
def load_colmap_reconstruction(
    ...
    *,
    ...
    if transform_file and os.path.exists(transform_file):
        with open(transform_file, "r") as fh:
            transform_data = json.load(fh)
...
def apply_sim3_to_points(points: np.ndarray, transform_data: dict) -> np.ndarray:
...
def apply_sim3_to_camera(cam: CameraInfo, transform_data: dict) -> CameraInfo:
