...
void write_completed_manifest(const Options& options, const Scene& scene,
    ...
    stream << std::setprecision(10)
           ...
           << (options.opacity_sh_enabled
                   ? "color-sh-plus-opacity-sh-v1"
                   : "color-sh-plus-scalar-opacity-v1")
           << "\",\n"
           << "    \"opacity_sh_enabled\": "
           << (options.opacity_sh_enabled ? "true" : "false") << ",\n"
           << "    \"opacity_sh_learning_rate_ratio\": "
           << (options.opacity_sh_enabled ? 0.05 : 0.0) << ",\n"
           ...
           << "    \"background_mode\": \""
           << json_escape(options.background_mode) << "\",\n"
           << "    \"loss_pixel_mask\": \""
           << json_escape(options.loss_pixel_mask) << "\",\n"
           << "    \"initial_ply\": "
           ...
           << (options.loss_pixel_mask == "all"
                   ? "\"linear_blend_of_0.8_all_pixel_l1_plus_0.2_dssim"
                     "_and_all_pixel_mse\",\n"
