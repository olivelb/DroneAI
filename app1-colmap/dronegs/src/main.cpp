...
int main(int argc, char** argv) {
    ...
    if (argc == 2 && std::string_view(argv[1]) == "--version") {
        std::cout
            << "DroneGS 0.5.0-dev.69 device-gaussian-compaction "
    ...
    try {
        ...
        std::cerr << "DroneGS 0.5.0-dev.69 uses an independent "
                     ...
                     "once per active pixel; dev61 rejects only splats whose "
                     "conservative screen-space support is entirely outside "
                     "the image before evaluating SH and exact covariance; "
                     "dev62 aligns scalar Adam on 16-lane groups and "
                     "normalizes rotations in the same kernel; dev63 "
                     "specializes SH Adam by active degree and interleaves "
                     ...
                     "traversal; dev65 makes directional opacity SH opt-in "
                     "and skips its forward, backward, and Adam work on the "
                     "default scalar-opacity path.\n";
        ...
        measurements.topology_telemetry = training.topology_telemetry;
